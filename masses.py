...
for unique_atom in set(monoisotopic["atom"]):
    ...
    subset.sort_values(by=["abundance"], ascending=False, inplace=True)
    # print(subset)
...
def calculate_monoisotopic_mass(molecule=""):
    mass = 0
    for i, a in enumerate(molecule):
        if a.isalpha() and a.upper() == a:
            ...
            if number == "":
                number = 1
            number = int(number)
            mass += mass_dict[atom]["abundant"] * int(number)
            print(atom, number, mass_dict[atom]["abundant"], mass_dict[atom]["abundant"] * int(number))
